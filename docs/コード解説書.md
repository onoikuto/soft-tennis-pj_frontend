...
void main() async {
  // ① Flutterエラーハンドリングを設定（ウィジェットツリー内のエラーもキャッチ）
  ...
  runZonedGuarded(() async {
    ...
    if (kIsWeb) {
      ...
      // Android/iOS: 標準のsqfliteを使用（追加の初期化は不要）
    }
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const SafeMainMenuScreen(),  // 最初に表示する画面（エラーハンドリング付き）
...
1. **`FlutterError.onError`**: Flutterウィジェットツリー内のエラーをキャッチ
2. **`runZonedGuarded`**: 予期しないエラー（非同期エラーなど）をキャッチ
3. **`ErrorWidget.builder`**: エラー発生時にカスタムエラー画面を表示
...
### 8.1 SubscriptionService（サブスクリプション管理）
...
class SubscriptionService {
  ...
  Future<bool> purchaseSubscription() async {
    ...
    // モバイル版での実装（将来実装時にin_app_purchaseを使用）
    await setSubscribed(true);
    return true;
  }
  
  // 購入履歴を確認してサブスクリプション状態を更新（将来実装）
...
### 8.2 広告表示（Google Mobile Ads）

```dart
// statistics_screen.dart での広告表示例
...
**備考**: 
- `main.dart` で `MobileAds.instance.initialize()` を呼び出して初期化
- `AndroidManifest.xml` にAdMob App IDを設定する必要がある
